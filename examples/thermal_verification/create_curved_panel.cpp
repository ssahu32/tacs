...
void write_stiffened_panel_axial( const char * file_name,
                                  ...
                                  double angle, double forced_disp, int write_shear_file ){
  ...
  for ( int i = 0; i < nx; i++ ){
    ...
    Xpanel[i] = Lx * cos(static_cast<double>(i) / static_cast<double>(nx-1) * M_PI);
  }

  for ( int i = 0; i < nx; i++ ){
    // Zpanel[i] = (0.1*Lx*i)/(nx-1);
    // double height = 0.002;
    // double height = 0.01;
    // Zpanel[i] = (4*height/pow(Lx,2)) * pow( Lx*i/(nx-1) - Lx/2, 2 );

    Zpanel[i] = Lx * sin(static_cast<double>(i) / static_cast<double>(nx-1) * M_PI);
  }
  ...
  for ( int j = 0, jj = 0; jj < nrepeat; jj++ ){
    ...
    //   Ysurf[j] = offset + (wb*k)/(3*(elem_order-1)*Nywb);
    //   // Ysurf[j] = offset;
    // }
  ...
  */
  ...
  for ( int jj = 0, j = 0; jj < nrepeat; jj++ ){
    ...
    //     fprintf(fp, "*       %16.9e%16d%16s%16d        \n",
    //             Zstiff[k] + Zpanel[i], coord_disp, spc, seid);
    //   }
    // }
  }
  ...
  */
  ...
  // Axial stiffened panel here

  ////////////////////////////////////////////////////////////////////////////
  // Case 1
  // Fixed cold end temperature BCs. Doesn't do anything without applied heat flux
  ...
  //             "SPC", 1, node, "1234", 0.0); // x, y, z, temp
  //   }
  //   else {
  //     // Surface base edge
  //     fprintf(fp, "%-8s%8d%8d%8s%8.6f\n",
  //             "SPC", 1, node, "134", 0.0); // x, z, temp
  //   }
  // }
...
int main( int argc, char * argv[] ){
  ...
  if (write_shear_file){
    write_stiffened_panel_axial("curved_panel.bdf",
                                Nx, nrepeat, Lx, b, wb, hs,
                                angle, forced_disp, write_shear_file);
  }
  else {
    write_stiffened_panel_axial("curved_panel.bdf",
                                Nx, nrepeat, Lx, b, wb, hs,
